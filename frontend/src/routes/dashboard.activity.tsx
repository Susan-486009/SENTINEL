...
function ActivityPage() {
  ...
  const activities: Array<{ type: string; title: string; description: string; timestamp: string; complaint: Complaint; icon: any }> = [];
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground">Timeline of your report submissions and status updates</p>
      </div>
      ...
      <div className="space-y-4">
        {sortedActivities.map((activity, index) => {
          ...
          return (
            <div key={index} className="rounded-2xl border border-border bg-card p-4 sm:p-5 hover:bg-muted/50 transition">
              <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                    <div>
                      <h3 className="font-semibold text-sm sm:text-base text-foreground">{activity.title}</h3>
                      <p className="text-xs mt-1 text-muted-foreground">{activity.description}</p>
                      {activity.complaint && (
                        <p className="text-xs mt-2 font-mono text-accent">
                          {activity.complaint.reference_id || activity.complaint.referenceId || 'N/A'}
                        </p>
                      )}
                    </div>
