    ...
    >
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground max-w-xl">
              Report academic, facility, or administrative concerns safely. Your voice drives transparency and change across LASUSTECH.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-1 rounded-2xl border border-border/80 bg-card/60 backdrop-blur-md p-4 text-xs font-mono text-muted-foreground shadow-sm max-w-max self-start md:self-center border-l-4 border-l-accent">
            ...
            <span className="mt-1">Matric ID: <span className="text-foreground font-semibold">{user?.matric || "N/A"}</span></span>
            <span>Role Type: <span className="text-foreground font-semibold capitalize">{user?.role}</span></span>
          </div>
        </div>
      </motion.div>
      ...
      >
        {[
          ...
          <motion.div
            key={index}
            whileHover={{ y: -4, shadow: "0 12px 24px -10px rgba(0,0,0,0.06)" }}
            className={`rounded-2xl border ${item.border} bg-card p-5 transition-all duration-300 shadow-soft`}
          >
      ...
      <div className="grid gap-8 lg:grid-cols-12">
        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="font-display text-xl font-bold tracking-tight">Recent Resolutions & Claims</h3>
            {list.length > 0 && (
              <Link 
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
            {list.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 text-center px-6">
                ...
                <h4 className="font-display text-lg font-bold text-foreground">No reports filed yet</h4>
                <p className="mt-2 text-xs text-muted-foreground max-w-sm leading-relaxed">
                  You haven't submitted any concerns yet. If you have an academic, IT, facility, or staff complaint, file it today.
                </p>
                <Link 
                  to="/submit" 
              ...
              <div className="divide-y divide-border/60">
                {list.slice(0, 5).map((complaint: Complaint) => {
                  ...
                  if (complaint.status === "in_review") badgeBg = "bg-indigo-500/10 text-indigo-600 border-indigo-500/20";
                  if (complaint.status === "resolved") badgeBg = "bg-emerald-500/10 text-emerald-600 border-emerald-500/20";
                  if (complaint.status === "rejected") badgeBg = "bg-rose-500/10 text-rose-600 border-rose-500/20";
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div className="space-y-1">
                          <div className="flex items-center gap-2.5">
                            <span className="font-mono text-xs font-bold text-accent select-all">{refCode}</span>
                            <span className="text-[10px] text-muted-foreground">•</span>
                            <span className="text-xs text-muted-foreground capitalize">{complaint.category.replace("-", " ")}</span>
                          </div>
                          <h4 className="font-semibold text-sm group-hover:text-primary transition line-clamp-1">{complaint.title}</h4>
                        </div>
                        <div className="flex items-center gap-3 self-start sm:self-center">
                          <span className={`inline-flex items-center rounded-lg border px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${badgeBg}`}>
                            {complaint.status.replace("_", " ")}
                          </span>
                          <span className="text-[10px] font-mono text-muted-foreground whitespace-nowrap">
                            {format(new Date(complaint.created_at || complaint.createdAt || Date.now()), "MMM dd, yyyy")}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 flex items-center gap-2">
                        ...
                        <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
                          {complaint.status === "resolved" ? "Completed" : complaint.status === "in_review" ? "Investigating" : "Pending"}
                        </span>
                      </div>
                    </div>
                  );
        ...
        >
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground leading-relaxed">Submit a concern with files/evidence directly to reviewers.</p>
              <Link 
                to="/submit" 
                className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-accent group-hover:underline"
              >
                Launch Submission <ArrowRight className="h-3 w-3 group-hover:translate-x-0.5 transition-transform" />
              </Link>
            </motion.div>
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground leading-relaxed">Track complaints directly by reference ID without logging in.</p>
              <Link 
                to="/track" 
                className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-accent group-hover:underline"
              >
                Open Case Tracker <ArrowRight className="h-3 w-3 group-hover:translate-x-0.5 transition-transform" />
              </Link>
            </motion.div>
            ...
            >
              ...
              <p className="mt-1 text-xs text-muted-foreground leading-relaxed">Update password, notification options, and theme controls.</p>
              <Link 
                to="/dashboard/settings" 
                className="mt-4 inline-flex items-center gap-1.5 text-xs font-bold text-accent group-hover:underline"
              >
                Account Settings <ArrowRight className="h-3 w-3 group-hover:translate-x-0.5 transition-transform" />
              </Link>
            </motion.div>
          </div>
          ...
          <div className="rounded-2xl border border-accent/20 bg-accent/5 p-5 border-l-4 border-l-accent shadow-soft">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="text-xs font-bold text-foreground uppercase tracking-wider">Sentinel Security Boundary</h4>
                <p className="mt-1.5 text-[11px] text-muted-foreground leading-relaxed">
                  LASUSTECH implements cryptographic correlation hashing on complaints. Stored uploads are automatically compressed and sanitized to preserve student confidentiality.
                </p>
              </div>
            </div>
          </div>
