...
function ForgotPage() {
  ...
  return (
    <AuthShell
      ...
      footer={
        <Link to="/login" className="inline-flex items-center gap-2 font-medium text-accent hover:underline">
      ...
      {sent ? (
        <div className="rounded-xl border border-success/30 bg-success/5 p-5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            If an account matches that email, we've sent reset instructions. The link expires in 30 minutes.
          </p>
        </div>
      ) : (
        <form
          ...
          className="space-y-5"
        >
          <Field label="University email" type="email" placeholder="you@lasustech.edu.ng" leading={<Mail className="h-4 w-4" />} required />
          <button
            type="submit"
