...
function ReportsPage() {
  ...
  if (error) {
    return (
      <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-6 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{error instanceof Error ? error.message : "An error occurred"}</p>
      </div>
    );
  }
  ...
  const statuses = [
    ...
    { value: "in_review", label: "In Review", count: complaints.filter(c => c.status === "in_review").length },
    { value: "resolved", label: "Resolved", count: complaints.filter(c => c.status === "resolved").length },
    { value: "rejected", label: "Rejected", count: complaints.filter(c => c.status === "rejected").length },
  ];
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0">
          {statuses.map((status) => (
            ...
            >
              {status.label} <span className="ml-1 inline text-[10px] font-bold">({status.count})</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:gap-4">
        {filteredComplaints.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-border p-8 sm:p-12 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">No {filterStatus !== "all" ? filterStatus : ""} reports</p>
          </div>
        ) : (
          filteredComplaints.map((complaint) => {
            let statusBg = "bg-amber-500/10 text-amber-600 border-amber-500/20";
            if (complaint.status === "in_review") statusBg = "bg-indigo-500/10 text-indigo-600 border-indigo-500/20";
            if (complaint.status === "resolved") statusBg = "bg-emerald-500/10 text-emerald-600 border-emerald-500/20";
            if (complaint.status === "rejected") statusBg = "bg-rose-500/10 text-rose-600 border-rose-500/20";
                ...
                <div className="space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-center flex-wrap">
                      <span className={`inline-flex items-center rounded-lg border px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${statusBg}`}>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-[10px] text-muted-foreground">
                    ...
                    <span>{format(new Date(complaint.created_at || complaint.createdAt || Date.now()), "MMM dd, yyyy")}</span>
                  </div>
