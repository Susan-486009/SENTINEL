...
function NotificationsPage() {
  ...
  const filteredNotifications = filterRead === "all"
    ? notifications
    : filterRead === "unread"
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col gap-4">
        <h1 className="font-display text-2xl sm:text-3xl font-bold tracking-tight">Notifications</h1>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0">
          ...
          >
            Unread {unreadCount > 0 && <span className="ml-1 inline text-[10px] font-bold">({unreadCount})</span>}
          </button>
          <button
      ...
      <div className="space-y-3">
        {filteredNotifications.length === 0 ? (
          <div className="rounded-2xl border-2 border-dashed border-border p-8 sm:p-12 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">No {filterRead !== 'all' ? filterRead : ''} notifications</p>
          </div>
        ) : (
          filteredNotifications.map((notification) => (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
                {/* Icon */}
                <div className={`flex-shrink-0 flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-lg ${
                  !notification.read
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] sm:text-xs font-mono text-muted-foreground mt-2">
                    {format(new Date(notification.created_at || notification.createdAt || Date.now()), "MMM dd, yyyy HH:mm")}
                  </p>
                </div>
              </div>
            </div>
